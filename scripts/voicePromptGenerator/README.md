...
This repository contains the voice prompt data and scripts and synthesised voice files, used by the OpenRTX firmware, this is a work in progress.
...
The speech audio files are converted to 8KHz sample rate, the files must be in this format, because currently OpenRTX only supports 8KHz codec2
sample rate.
Once the speech files have been downloaded, they are converted to codec2 format using ffmpeg.
Once all files have been processed to codec2, the script packages the files into a single Voice Prompt pack binary file which must be uploaded to the radio.

## Please note!

The words in the wordlist.csv file must be synchronized with the source code as follows:
Strings up to and including `PROMPT_RIGHT_BRACE` correspond to voice prompts defined in the `voicePrompt_t` enum in `voicePrompts.h`.
